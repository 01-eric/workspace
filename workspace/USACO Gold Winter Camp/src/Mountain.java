...
public class Mountain {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < measurements.length; i++) {
			...
			else if (i < measurements.length - 1 && measurements[i + 1] == measurements[i - 1]) dp_fwd[i] = dp_fwd[i - 1] + 1;
			...
			else if (i < measurements.length - 1 && measurements[measurements.length - 2 - i] == measurements[measurements.length - 1 - i]) dp_rev[dp_rev.length - 1 - i] = dp_rev[dp_rev.length - i] + 1;
